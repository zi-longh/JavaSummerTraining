  ...
  <div>
      ...
      <div class="text item">
        ...
        <div>
          ...
          >
            ...
            <el-table-column label="供货日期">
              <template v-slot="scope">
                {{ scope.row.supplierDate }}
              </template>
            </el-table-column>
            <el-table-column label="供货量">
              <template v-slot="scope">
                {{ scope.row.supplierSum }}
              </template>
            </el-table-column>
    ...
    >
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="供货商编号" prop="sup_supplierNum">
          <el-select
            v-model="ruleForm.sup_supplierNum"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item label="日期" prop="supplierDate">
          <el-input v-model="ruleForm.supplierDate"></el-input>
        </el-form-item>
        <el-form-item label="供货量" prop="supplierSum">
          <el-input v-model="ruleForm.supplierSum"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      ...
      >
        <el-form-item label="供货商编号" prop="sup_supplierNum">
          <el-select
            v-model="addForm.sup_supplierNum"
            placeholder="请选择"
          >
            <el-option
              v-for="item in suppliersNumArr"
        ...
        <el-form-item label="日期" prop="supplierDate">
          <el-input v-model="addForm.supplierDate"></el-input>
        </el-form-item>

        <el-form-item label="供货量" prop="supplierSum">
          <el-input v-model="addForm.supplierSum"></el-input>
        </el-form-item>
      </el-form>
      <template v-slot:footer>
        <span class="dialog-footer">
...
import http from '../request/provide'
import goods from '../request/goods'
import supplier from '../request/supplier'
export default {
  components: {
    ElIconEdit,
    ElIconDelete,
  ...
  methods: {
    ...
    async getNumArr() {
      ...
      const res2 = await supplier.getSuppliersNum()
      if (!res2.status) this.suppliersNumArr = res2.data
    },
    //改变每页大小的方法
    ...
    handleEdit(msg) {
      ...
      // msg['office']
      this.getNumArr()
      const { id, sup_supplierNum, goo_goodsNum, supplierDate, supplierSum } =
    ...
    handleDelete(supplierNum) {
      ...
      this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
        ...
        .then(async () => {
          const res = await http.deleteProvideById({ id: supplierNum })
          if (!res.status) {
    ...
    handleAdd() {
      this.getNumArr()
      console.log(this.suppliersNumArr)
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          if (formName == 'addForm') {
            ...
            const res = await http.addProvide(this.addForm)
            if (!res.status) {
              this.$message.success('添加供货信息成功！')
              this.currentPage = 1
              this.getAllInfo()
              this.addDialogVisible = false
              this.formSearch = {}
            } else {
              this.$message.error('添加供货信息失败！')
              this.dialogVisible = false
            }
          } else {
            const result = await http.updateProvideById(this.ruleForm)
            if (!result.status) {
              this.$message.success('修改供货信息成功！')
              this.dialogVisible = false
              this.currentPage = 1
              this.getAllInfo()
              this.formSearch = {}
            } else {
              this.$message.error('修改供货信息失败！')
              this.dialogVisible = false
            }
          }
        } else {
    ...
    //  获取所有信息
    async getAllInfo() {
      const res = await http.getAllProvidesInfo()
      if (!res.status) {
        this.tableData = this.tableNewData = []
